#include <hop/MFSH.hpp>

namespace hop {

std::vector<double> MFSH::compute_probabilities(const double & dt) const {
    ...
    // In 2-state case minimum flux is equivalent to the well-defined unique flux
    if (NStates_ == 2) {
        if (active_state_ == 0) {
            probabilities[0] = 0.0;
    ...
    // Calculate minimum flux for other cases
    else {
...
MFSH::MFSH(const int64_t & _NStates, const at::Tensor & _mass,
...
void MFSH::step(const double & dt) {
...
void MFSH::step_1D(const double & dt) {
    // Propagate nuclear coordinate and momentum
    propagate_nucleus(dt);
    // Propagate electronic wave function
    propagate_electron(dt);
